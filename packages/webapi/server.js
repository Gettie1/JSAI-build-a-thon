...
const chatModel = new AzureChatOpenAI({
  ...
  azureOpenAIApiVersion: "2024-04-01-preview", // In target url: ...<VERSION>
  temperature: 1,
  maxTokens: 4096,
});
...
async function loadPDF() {
  ...
  try {
    ...
    console.log(`PDF loaded successfully. Created ${pdfChunks.length} chunks.`);
    return pdfText;
  } catch (error) {
...
app.listen(PORT, () => {
  ...
  if (!process.env.AZURE_INFERENCE_SDK_ENDPOINT) {
    console.error('Missing AZURE_INFERENCE_SDK_ENDPOINT environment variable');
  } else {
    console.log('Azure endpoint configured:', process.env.AZURE_INFERENCE_SDK_ENDPOINT);
  }
  
  if (!process.env.AZURE_INFERENCE_SDK_KEY) {
    console.error('Missing AZURE_INFERENCE_SDK_KEY environment variable');
  } else {
    console.log('Azure API key configured');
  }
